...
public class ImageVisualComponent extends VisualComponent
{
	...
	public BufferedImage scale(int size)
	{
		...
		{
			scaledImage = Util.imageToBufferedImage(originalImage.getScaledInstance(-1, size, Image.SCALE_FAST));
		//if it is long, shortens width to specified size
		} else {
			scaledImage = Util.imageToBufferedImage(originalImage.getScaledInstance(size, -1, Image.SCALE_FAST));
		}
		this.size = size;
		return scaledImage;
	}
